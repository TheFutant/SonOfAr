...
- **Events:** `session_start` (once per page load), `new_game`, `ending` (label = `endingId`), `detour` (entering the optional Drive). Add events by extending the `UsageEvent` union in `usage.ts` **and** the `map $arg_evt $usage_evt` whitelist in `nginx.conf` — the whitelist guards against log injection, so an event not in both places is silently dropped.
