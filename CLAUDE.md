...
- `src/state/reducer.ts` — the `Action` type and `reducer` function. Action types: `choose`, `load`, `new`, `set-flag`. `new` deliberately preserves `completedEndings`, `chaosMode`, `writersRoomMode`, `soundOn`, `seenPrologue` across resets.
