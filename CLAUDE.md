...
The **prologue** is a skippable typographic "comic" cold-open that runs before a new game (and is replayable via the title's `▸ Prologue` link). It's story-as-data: panels live in `src/data/prologue.ts`, rendered by `src/components/Prologue.tsx` — pure type + CSS over the `EmberBackground`, **zero illustration assets** so it stays offline-clean. Each `ProloguePanel` has an optional `art` path (under `/public`) as a seam for future AI-generated panel art: set it and the component renders the image behind the caption, no structural change. Keep any art canon-safe (Maebie is a living partner; the Editor's voice; Pearson is not yet "Reason").
